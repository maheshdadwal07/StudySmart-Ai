    ...
    <div style={{ position: 'fixed', top: 0, left: 0, right: 0, bottom: 0, backgroundColor: 'rgba(0,0,0,0.5)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 100 }}>
      <div style={{ backgroundColor: 'var(--card)', padding: '24px', borderRadius: '12px', width: '400px', maxWidth: '90%' }}>
        ...
        <div style={{ display: 'flex', justifyContent: 'flex-end', gap: '12px' }}>
          ...
          <Button variant="primary" onClick={() => onConfirm(name)} disabled={isLoading || !name.trim()}>
...
export default function UploadsPage() {
  ...
  const totalSizeBytes = documents.reduce((acc, doc) => acc + (doc.file_size_bytes || 0), 0);
  const totalMb = totalSizeBytes / (1024 * 1024);
  const storageUsageStr = totalMb >= 1.0 ? `${totalMb.toFixed(1)} MB` : totalSizeBytes > 0 ? `${Math.round(totalSizeBytes / 1024)} KB` : '0 KB';
            ...
            <tbody>
              {loading ? (
                ...
                documents.map((item) => {
                  ...
                  const sizeStr = mb >= 1.0 ? `${mb} MB` : `${Math.round(bytes / 1024)} KB`;
